module fassert_open_pad
    use :: fassert_kit
    use :: enumul_open_pad
    implicit none
    private
    public :: is_equal_enum_open_pad
    public :: output_on_failure_enum_open_pad_to_str

    character(*), parameter :: fmt = '('//fmt_indent//',A,A)'
    character(*), parameter :: type_mismatch_expected = "Type mismatch: `expected` is not enum_open_pad"
    character(*), parameter :: type_mismatch_actual = "Type mismatch: `actual` is not enum_open_pad"

contains
    pure logical function is_equal_enum_open_pad(actual, expected)
        implicit none
        class(*), intent(in) :: actual
        class(*), intent(in) :: expected

        is_equal_enum_open_pad = .false.
        select type (actual); type is (enum_open_pad)
            select type (expected); type is (enum_open_pad)

                is_equal_enum_open_pad = &
                    all([actual%enum == expected%enum, &
                         actual%expr == expected%expr])

            end select
        end select
    end function is_equal_enum_open_pad

    pure subroutine output_on_failure_enum_open_pad_to_str(actual, expected, output_message)
        implicit none
        class(*), intent(in) :: actual
        class(*), intent(in) :: expected
    ...
    end subroutine output_on_failure_enum_open_pad_to_str
end module fassert_open_pad

module test_open_unitTests_pad
    use, intrinsic :: iso_fortran_env
    use :: fassert
    use :: fassert_open_pad
...
contains
    ...
    subroutine assignment_op_for_enum_open_pad_assigns_enum_and_char_expr(error)
        ...
        call expect_equal(pad, open_pad%no, &
                          "assigned enum should equal to rhs", &
                          comparator=is_equal_enum_open_pad, &
                          verbose_message_writer=output_on_failure_enum_open_pad_to_str, &
    ...
    subroutine default_open_pad_enum_is_yes(error)
        ...
        call expect_equal(default_open_pad, open_pad%yes, &
                          "default pad specifier shoud equal to the yes", &
                          comparator=is_equal_enum_open_pad, &
                          verbose_message_writer=output_on_failure_enum_open_pad_to_str, &
                          stat=stat, output_message=msg)
        call check(error, stat, msg)
        ...
        call expect_equal(default, default_open_pad, &
                          "`get_open_pad_default` should return `default_open_pad`", &
                          comparator=is_equal_enum_open_pad, &
                          verbose_message_writer=output_on_failure_enum_open_pad_to_str, &
                          stat=stat, output_message=msg)
        call check(error, stat, msg)
    ...
    subroutine optval_returns_default_when_x_is_not_presented(error)
        ...
        call expect_equal(y, default_open_pad, &
